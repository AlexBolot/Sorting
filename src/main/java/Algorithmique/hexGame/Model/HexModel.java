...
public class HexModel extends Observable {
     ...
     * <p>
     * La détection de la victory est décomposée en trois méthodes. La méthode researchVictory est appellé après un clique du player. On va
     * tester la victory pour le player qui a cliqué. De cette facon on ne fait pas de test inutile en testant à chaque clique la victory pour
     * les deux joueurs, onne test que pour le player qui a cliqué.
     * <p>
     * La méthode researchVictory fait appel aux méthodes researchNeighborCells et researchWinner. La méthode researchNeighborCells retourne
     * une arrayList de toutes les cellules voisines de même couleur que la cellule passé en paramètre. A partir des cellules voisines on fait
     * appel à la méthode rechercheGagnant qui va parcourir l'arrayList des cellules voisines. Pour chaque cellule présente dans l'arayList on
    ...
    public void researchVictory(int i, int j) {
        ...
        if (victory) {
            if (startingCell.getColor() == Color.BLUE) setWinner(Color.BLUE);

            else setWinner(Color.RED);
        }
        ...
        if (!neighborCells.isEmpty()) {
            ...
            for (Cell cell : neighborCells) {
                ...
                for (Cell cells : nextNeighborCells) {
                    if (cells.getColor() == Color.BLUE && cells.getX() >= grid.getNbLines() - 2) victory = true;
                    if (cells.getColor() == Color.RED && cells.getY() >= grid.getNbColumns() - 2) victory = true;
                }

                // Si le player ne gagne pas on relance l'algorithme de recherche de gagnant
                if (!victory) victory = researchWinner(nextNeighborCells);
            }
        }
    ...
    public ArrayList<Cell> researchNeighborCells(Cell cellForResearch) {
        ...
        for (Cell cell : tmpCells) {
            if (cell != null && grid.isNotPast(cell) && cell.getColor() == color) {
